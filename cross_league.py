...
all - see build_statpack.py notes), so this module can't derive one
empirically. Rather than inventing an unjustified "Premier League is
worth 1.4x League Two" style number, it uses two things that ARE
...
This is a considered best-effort, not a rigorous solution - cup
predictions built on this should be treated with real caution, which
is why they're visibly flagged as cross-division in the output.
"""
...
def cross_league_expected_values(
    ...
    reference point, and each side gets nudged by their league-position
    multiplier.
    """
    ...
    lam_home = shared_home_baseline * home_attack * away_defense * home_position_mult
    lam_away = shared_away_baseline * away_attack * home_defense * away_position_mult
